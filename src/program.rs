...
pub fn parse_program_header<'a, B: Buffer + 'a>(
    input: B,
    header: Header<'a, B>,
    index: u16,
) -> Result<ProgramHeader<'a, B>, ParseError<B::Error>> {
    let pt2 = &header.pt2;
    ...
    match header.pt1.class() {
        Class::ThirtyTwo => Ok(ProgramHeader::Ph32(
            input
                .offset(start)
                .truncate(size)
                .read()
        ...
        Class::SixtyFour => Ok(ProgramHeader::Ph64(
            input
                .offset(start)
                .truncate(size)
                .read()
...
#[derive(Debug, Clone)]
pub struct ProgramIter<'b, 'a: 'b, B: Buffer + 'a> {
    pub file: &'b ElfFile<'a, B>,
    pub next_index: u16,
}

impl<'b, 'a, B: Buffer + 'a> Iterator for ProgramIter<'b, 'a, B> {
    type Item = ProgramHeader<'a, B>;
...
pub enum ProgramHeader<'a, B: Buffer + 'a> {
    Ph32(B::Ref<'a, ProgramHeader32>),
    Ph64(B::Ref<'a, ProgramHeader64>),
}
...
impl<'a, B: Buffer + 'a> ProgramHeader<'a, B> {
    ...
    ) -> Result<SegmentData<'a, B>, ParseError<B::Error>> {
        match *self {
...
impl<'a, B: Buffer + 'a> fmt::Display for ProgramHeader<'a, B> {
...
macro_rules! ph_impl {
    ($ph: ident) => {
        impl $ph {
                        ...
                        Ok(match typ {
                            ...
                            Type::Dynamic => {
                                ...
                                match elf_file.header.pt1.class() {
                                    Class::ThirtyTwo => SegmentData::Dynamic32(
                                        data.read_array().map_err(ParseError::Io)?,
                                    ),
                                    Class::SixtyFour => SegmentData::Dynamic64(
                                        data.read_array().map_err(ParseError::Io)?,
                                    ),
                                    Class::None | Class::Other(_) => unreachable!(),
                                }
                            ...
                            Type::Note => {
                                ...
                                match elf_file.header.pt1.class() {
                                    ...
                                    Class::SixtyFour => {
                                        let header =
                                            data.truncate(12).read().map_err(ParseError::Io)?;
            ...
            pub fn raw_data<'a, B: Buffer + 'a>(&self, elf_file: &ElfFile<'a, B>) -> B {
                assert!(self
                    .get_type()
                ...
                if self.file_size == 0 {
                    ...
                    elf_file
                        ...
                        .truncate(self.file_size as usize)
                }
            }
        }
...
pub fn sanity_check<'a, B: Buffer + 'a>(
    ...
    match ph {
        ProgramHeader::Ph32(ph) => {
            check!(
                size_of_val(&*ph) == header.pt2.ph_entry_size() as usize,
